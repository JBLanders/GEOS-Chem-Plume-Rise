...
!
! !DESCRIPTION: Module HCOX\_Plume\_Agg\_Mod implements a thermodynamic plume
! rise model for wildfire emissions in HEMCO. It reads 2D fire emissions (CO),
! fire energy (TFC), and burned area (GrowthArea/BurnAreaTot) from HEMCO, and
! then distributes them using the Anderson et al (2011) scheme.
!
! This version (agg) fixes Issue E from Validate.md: rather than computing
! energy as TFC*BurnAreaTot (which uses cumulative area but ignores TFC
! history), it computes fresh per-timestep energy from TFC*GrowthArea and
! accumulates it in Inst%Qo across timesteps, matching the CFFEPS C code
! (Qt = Qplume_current + Qo_prior each timestep). Qo is reset to zero when
! both GrowthArea and CO_ij are zero (fire has gone out).
!\\
!\\
...
!
  ...
  TYPE :: MyInst
    ...
    REAL(sp), ALLOCATABLE :: Qo(:,:)                      ! accumulated fire energy [J] — persists between timesteps
...
!
    ...
    REAL(sp), ALLOCATABLE :: save_Qt(:,:)                          !diagn: accumulated fire energy Qt (= Qplume + Qo_prior)
    REAL(sp), ALLOCATABLE :: save_QPlume(:,:)                      !diagn: per-timestep fire energy Qplume
    REAL(sp), ALLOCATABLE :: save_BurnAreaTot(:,:)                 !diagn: BurnAreaTot read each timestep
    REAL(sp), ALLOCATABLE :: save_TFC(:,:)                         !diagn: TFC input used in energy calc
    REAL(sp), ALLOCATABLE :: save_GrowthArea(:,:)                  !diagn: GrowthArea input used in energy calc
    REAL(sp), ALLOCATABLE :: save_Qatm(:,:)                        !diagn: atmospheric work Qatm at convergence
    ...
    ! Allocate accumulated energy array on first call; initialize to zero so
    ! the first timestep starts with no prior energy (cold start).
    IF ( .NOT. ALLOCATED(Inst%Qo) ) THEN
       ALLOCATE( Inst%Qo(NX,NY) )
       Inst%Qo = 0.0_sp
    ENDIF
    ...
    ALLOCATE( save_TFC(NX,NY) )
    ALLOCATE( save_GrowthArea(NX,NY) )
    ALLOCATE( save_Qatm(NX,NY) )
    ...
    DO I = 1, NX
       ...
       save_TFC(I,J)         = TFC_ij
       save_GrowthArea(I,J)  = GrowthArea_ij
       ...
       DO WHILE ( (ii < IMAX_CF) .AND. (ABS(ddz) >= 1.0_sp) .AND. (dz <= IMAX_CF) )
          ...
          ! Iterate dz using accumulated energy Qt
          IF ( Qatm > Qt ) THEN
             ddz = 0.5_sp * ddz
             dz  = dz - ddz
          ELSE
       ...
       ! Save atmospheric work at convergence (should ~= Qt; off by one bisection
       ! step since Qatm is computed before the final dz update)
       save_Qatm(I,J) = Qatm

       ! Update accumulated energy for next timestep
       Inst%Qo(I,J) = Qt
    ...
    CALL Diagn_Update( HcoState, cName='save_Qt',          Array2D=save_Qt,          RC=RC )
    ...
    CALL Diagn_Update( HcoState, cName='save_TFC',         Array2D=save_TFC,         RC=RC )
    CALL Diagn_Update( HcoState, cName='save_GrowthArea',  Array2D=save_GrowthArea,  RC=RC )
    CALL Diagn_Update( HcoState, cName='save_Qatm',        Array2D=save_Qatm,        RC=RC )
...
!
    ...
    CALL Diagn_Create( HcoState, &
                       ...
                       RC = RC )
    IF ( RC /= HCO_SUCCESS ) THEN
       CALL HCO_ERROR( 'Cannot create save_Qt diagnostic', RC, THISLOC=LOC )
       RETURN
    ENDIF
    ...
    CALL Diagn_Create( HcoState, &
                       ...
                       RC = RC )
    IF ( RC /= HCO_SUCCESS ) THEN
       CALL HCO_ERROR( 'Cannot create save_TFC diagnostic', RC, THISLOC=LOC )
       RETURN
    ENDIF

    CALL Diagn_Create( HcoState, &
                       cName = 'save_GrowthArea', &
                       ExtNr = Inst%ExtNr, &
                       SpaceDim = 2, &
                       OutUnit = 'km2', &
                       OutOper = 'Mean', &
                       ...
                       RC = RC )
    IF ( RC /= HCO_SUCCESS ) THEN
       CALL HCO_ERROR( 'Cannot create save_GrowthArea diagnostic', RC, THISLOC=LOC )
       RETURN
    ENDIF
    ...
    CALL Diagn_Create( HcoState, &
                       ...
                       RC = RC )
    IF ( RC /= HCO_SUCCESS ) THEN
       CALL HCO_ERROR( 'Cannot create save_Qatm diagnostic', RC, THISLOC=LOC )
       RETURN
    ENDIF
    ...
    IF ( HcoState%amIRoot ) THEN
       msg = 'Using HEMCO extension: Plume (thermodynamic plume rise with Qo accumulation)'
...
!BOC
    ...
    IF ( ASSOCIATED(Inst) ) THEN
       ...
       IF ( ALLOCATED(Inst%Qo)            ) DEALLOCATE(Inst%Qo)
